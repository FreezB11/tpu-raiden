      ...
      worker_threads_.emplace_back([this, req_id, uuid, remote_endpoint,
        ...
        try {
          if (load_plan.num_blocks == 0) {
            ...
            TensorList local_views = LayerViews(
                slot_idx, static_cast<int64_t>(load_plan.num_blocks));
            if (response.num_layers != local_views.size()) {
              throw std::runtime_error(
                  "remote layer descriptor count mismatch");
            }
            for (size_t i = 0; i < local_views.size(); ++i) {
              ...
              descriptor_ms += DurationMs(descriptor_start,
                                          std::chrono::steady_clock::now());
              if (layer.len != static_cast<uint64_t>(local_views[i].nbytes())) {
                throw std::runtime_error(
  ...
  RaidenStageResult IssueH2D(int64_t slot_idx, int64_t num_blocks,
    ...
    for (size_t i = 0; i < kv_transfer_->num_layers(); ++i) {
      ...
      future->Add(ValueOrThrow(
          ...
          kv_transfer_->H2dFrom(i, host_views[i].data_ptr(),
                                host_views[i].nbytes(), transfer_spec)));
    }
    return {.future = std::move(future),
            .host_views = std::move(host_views),
